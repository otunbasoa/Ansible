...
# Create database
sudo -i -u postgres psql -c "CREATE DATABASE goddb WITH ENCODING 'UTF8' TEMPLATE template0"

# Create User
sudo -i -u postgres psql -c "CREATE USER god WITH ENCRYPTED PASSWORD 'god3000@'"

# Grant user privilege on database
sudo -i -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE goddb to god"
